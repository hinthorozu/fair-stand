...
Bu belge `TV_42` için `Version2` runtime kodunda çalışan TV / wall-overlay akışını toplar.
...
`duplicateModuleState()` state'i JSON clone eder, yeni module `id` üretir ve varsa strip/face/surface kimliklerini yeniler. `TV_42` için clone edilen normal nested state alanları korunur.
...
`buildProjectSnapshot()` bütün `currentModules` dizisini JSON clone ile proje snapshot'ındaki `modules` alanına yazar. `TV_42` state'i placement ve nested state alanlarıyla birlikte burada saklanır.
...
`moduleDragSidebar.js` içinde `TV_42` katalog grubu **Elektronik & Aydınlatma** altında yer alır. Catalog card drag akışı `Shift+R` ile behavior rotation step'ini kullanır; ghost/placement preview `scene3d.previewCatalogModuleDrag()` yoluna gider.
