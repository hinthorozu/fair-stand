...
import { furniture_bar_stool_classic_DIMENSIONS, MODULE_CATALOG } from '../src/catalog.js';
...
test('bar taburesi 50 x 50 x 80 cm ölçülerini kullanır', () => {
  assert.deepEqual(furniture_bar_stool_classic_DIMENSIONS, { widthCm: 50, depthCm: 50, heightCm: 80 });
  assert.equal(MODULE_CATALOG.furniture_bar_stool_classic.widthCm, 50);
  assert.equal(MODULE_CATALOG.furniture_bar_stool_classic.depthCm, 50);
  assert.equal(MODULE_CATALOG.furniture_bar_stool_classic.heightCm, 80);
