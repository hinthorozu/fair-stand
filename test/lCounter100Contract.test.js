...
test('desk_banko_100_L catalog contract',()=>{const item=MODULE_CATALOG.desk_banko_100_L;assert.equal(item.itemKey,'desk_banko_100_L');assert.equal(item.type,'counter');assert.equal(item.shape,'L');assert.equal(item.widthCm,100);assert.equal(item.depthCm,100);assert.ok(MODULE_CATALOG_KEYS.includes('desk_banko_100_L'));});
