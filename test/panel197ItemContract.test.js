...
test('panel_197 migration does not migrate neighboring straight or corner panel Items', () => {
  for (const itemKey of ['panel_48_5', 'panel_98', 'panel_147_5', 'panel_corner_192']) {
    const item = getProductionItem(itemKey);
    assert.equal(item.itemKey, undefined, itemKey);
    assert.equal(item.partId, itemKey);
  }

  assert.equal(getStraightWallRecipe(200).variants.innerCornerPanelPartId, 'panel_corner_192');
});
