...
def _positive_decimal(value: object, *, field: str) -> Decimal:
    try:
        ...
        raise SettingsAdminError(f"{field} must be a number") from exc
    if number <= 0:
        raise SettingsAdminError(f"{field} must be greater than 0")
    return number
...
def _positive_int(value: object, *, field: str) -> int:
    try:
        ...
        raise SettingsAdminError(f"{field} must be an integer") from exc
    if number <= 0:
        raise SettingsAdminError(f"{field} must be greater than 0")
    return number
...
class AdminSettingsService:
    ...
    def update_stand_dimensions(
        ...
        *,
        ...
        depth_m: object,
        strip_count: object,
        strip_height_m: object,
        frame_width_m: object,
        frame_depth_m: object,
    ) -> dict:
        depth = _positive_decimal(depth_m, field="depth_m").quantize(Decimal("0.001"))
        strips = _positive_int(strip_count, field="strip_count")
        strip_height = _positive_decimal(strip_height_m, field="strip_height_m").quantize(
            Decimal("0.001")
        )
        frame_width = _positive_decimal(frame_width_m, field="frame_width_m").quantize(
            Decimal("0.001")
        )
        frame_depth = _positive_decimal(frame_depth_m, field="frame_depth_m").quantize(
            Decimal("0.001")
        )
        height = (Decimal(strips) * strip_height).quantize(Decimal("0.001"))
        provided_height = _positive_decimal(height_m, field="height_m").quantize(Decimal("0.001"))
        if provided_height != height:
            raise SettingsAdminError(
                f"height_m must equal strip_count × strip_height_m ({height})",
            )
        ...
        row.strip_height_m = strip_height
        row.frame_width_m = frame_width
        row.frame_depth_m = frame_depth
        row.updated_at = now
        self._session.flush()
    ...
    def update_runtime_settings(
        ...
        *,
        ...
        max_mb = _positive_int(max_image_upload_mb, field="max_image_upload_mb")
