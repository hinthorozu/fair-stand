...
function replaceOnce(path, oldText, newText) {
  const src = read(path);
  if (!src.includes(oldText)) throw new Error(`Pattern not found in ${path}: ${oldText.slice(0, 120)}`);
  write(path, src.replace(oldText, newText));
}
...
// designState.js: inherit TV state and preserve video-wall grid metadata.
{
  const path = 'src/designState.js';
  let s = read(path);
  if (!s.includes('videoWallRows: Math.max(1')) {
    s = s.replace(
...
// main.js: pass full TV catalog descriptor into state creation.
{
...
// scene3d.js: replace TV renderer with a grid-aware renderer. Ordinary TVs remain 1x1.
{
  const path = 'src/scene3d.js';
  let s = read(path);
  const start = s.indexOf('function createTvModule(moduleState, moduleIndex) {');
  const end = s.indexOf('\nfunction createMiniFridgeTopLabel(', start);
  if (start < 0 || end < 0) throw new Error('createTvModule boundaries not found');
...
// moduleDragSidebar.js: only video walls get the 2x2 grid preview.
{
  const path = 'src/moduleDragSidebar.js';
  let s = read(path);
  if (!s.includes('.module-drag-tv.is-video-wall::before')) {
    s = s.replace(
...
console.log('2x2 video wall patch applied');
