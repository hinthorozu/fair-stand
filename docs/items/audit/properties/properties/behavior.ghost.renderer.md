...
**Özellik ID:** `behavior.ghost.renderer`
**İnsan tarafından anlaşılır adı:** Modül davranışı: ghost.renderer
**Kategori:** davranis
**Veri tipi:** string
**Birim:** yok / birimsiz
**İzin verilen değerler / enum / aralık:** TSV'de görülen değerler (canlı dump): `module-silhouette`
...
- `src/moduleBehavior.js` (dosya düzeyi) :7 [write-or-literal]

## Okuyan yerler

- `test/globalSilhouetteGhost.test.js` (dosya düzeyi) :8 [test]
- `test/moduleBehaviorContract.test.js` (dosya düzeyi) :45 [test]
- `test/tv42Module.test.js` `for` :65 [test]
