...
**Özellik ID:** `contract.renderer.mode`
**İnsan tarafından anlaşılır adı:** Sözleşme renderer sınıfı
**Kategori:** sozlesme
**Veri tipi:** string
**Birim:** yok / birimsiz
**İzin verilen değerler / enum / aralık:** TSV'de görülen değerler (canlı dump): `procedural` · `model` · `specialized-media` · `specialized-overlay` · `procedural-or-specialized` · `procedural-or-model`
...
- `src/moduleContracts.js` `MODULE_CONTRACT_PROFILES` :31 [write-or-literal]
