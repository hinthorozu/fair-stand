...
**Özellik ID:** `factory.fn`
**İnsan tarafından anlaşılır adı:** State factory fonksiyon adı
**Kategori:** audit-factory
**Veri tipi:** string
**Birim:** yok / birimsiz
**İzin verilen değerler / enum / aralık:** TSV'de görülen değerler (canlı dump): `createUprightModuleState` · `createProfileModuleState` · `createShelfModuleState` · `createCoatRackModuleState` · `createKettleModuleState` · `createMiniFridgeModuleState` · `createPlasticTrashBinModuleState` · `createBeigeSofaSetModuleState` · `createSofaSingleClassicModuleState` · `createSofaDoubleClassicModuleState` · `createCoffeeTableClassicModuleState` · `createEamesTableChairSetModuleState` (+14)
...
- `test/wallFlatPanelItemsContract.test.js` (dosya düzeyi) :6 [test]
- `test/wallMediaItemsContract.test.js` (dosya düzeyi) :5 [test]
- `test/wallMediaItemsContract.test.js` `for` :80 [test]
- `test/wallSeparatorItemsContract.test.js` (dosya düzeyi) :5 [test]
- … test/e2e ek 1 eşleşme (src listesi tam).
