...
## Bileşim / BOM
`composition.mode=recipe`, `moduleType=base-wall`; recipe lookup `item.dimensions.widthCm=150`. Miktar tek kaynak: `src/moduleRecipes.js` `base-wall:150` (`base-wall-150`). Alt satırları Item kartında kopyalanmaz; mevcut recipe miktarları bu migration’da değişmez. Raw BOM UI dokunulmadı.
