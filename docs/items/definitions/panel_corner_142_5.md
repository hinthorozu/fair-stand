...
Bu belge `panel_corner_142_5` Item'ının canonical Item Contract tanımını ve doğrulanmış inner-corner BOM replacement kuralını kaydeder. Pre-migration kod haritası `docs/items/current-system/panel_corner_142_5.md` içindedir.

## 1. Kimlik ve sınıflandırma

| Alan | Canonical değer | Durum |
|---|---|---|
| `itemKey` | `panel_corner_142_5` | VAR |
| `type` | `panel` | VAR |
| Yapı | Tekil Item | VAR |
...
| dimensions | `142,5 × 47 × 0.8 cm` | VAR |

Canonical production kaydı `src/productionParts.js` içindeki `PRODUCTION_PARTS.panel_corner_142_5` kaydıdır. Legacy `partId` kaldırılmıştır; ikinci/paralel ürün kimliği oluşturulmamıştır.

## 2. Factory / catalog

- Project-instance factory: **UYGULANMIYOR**. `panel_corner_142_5` bağımsız scene/project instance değildir.
...
`panel_corner_142_5`, `panel_147_5` Item'ının child Item'ı değildir. İkisi ayrı Tekil Item'dır:

```text
panel_147_5          -> panelRole = straight
panel_corner_142_5   -> panelRole = inner-corner
```
...
Canonical `innerCornerPanelItemKey = panel_corner_142_5` referansı tam **4** doğrulanmış 150 cm recipe'de bulunur:

| Recipe | Straight panel | Quantity | Inner-corner sonucu |
|---|---|---:|---|
| `wall-straight-150` | `panel_147_5` | 7 | `panel_corner_142_5 × 7` |
| `shelf-wall-150-2` | `panel_147_5` | 7 | `panel_corner_142_5 × 7` |
| `shelf-wall-150-3` | `panel_147_5` | 7 | `panel_corner_142_5 × 7` |
| `base-wall-150` | `panel_147_5` | 7 | `panel_corner_142_5 × 7` |

Legacy `innerCornerPanelPartId` referansı bu recipe'lerde kaldırılmıştır.

## 7. BOM resolver ve quantity ownership

Canonical recipe/BOM resolver `src/moduleRecipes.js` içindedir. Normal konfigürasyonda mevcut `panel_147_5` satırı korunur. Caller doğrulanmış relationship/configuration bilgisini:
...
- Recursive BOM: **UYGULANMIYOR**. `panel_corner_142_5` Tekil Item'dır ve alt Item listesi yoktur.

## 9. Regression sözleşmesi

`test/cornerPanelsItemContract.test.js` ve ilgili recipe testleri şunları kilitler:

1. canonical `itemKey = panel_corner_142_5` vardır ve legacy `partId` yoktur,
2. production metadata `142,5 × 47 × 0.8`, `panel`, `adet`, `inner-corner`, `150` olarak korunur,
3. tam `4` doğrulanmış parent recipe canonical `innerCornerPanelItemKey` kullanır,
4. inner-corner çözümünde `panel_147_5 × N` tamamen kaldırılır ve `panel_corner_142_5 × N` **aynı miktarla** gelir,
...
replacement rule = panel_147_5 × N -> panel_corner_142_5 × N (1:1)
