...
`src/items.js > COMPOSITE_ITEMS.desk_banko_100` canonical itemKey/name/type/dimensions sahibidir. Bileşik Item; `itemKey = desk_banko_100`. Type ailesi `counter`; straight genişlik 100 cm. Doğrulanmış ölçüler: `widthCm=100`, `depthCm=50`, `heightCm=100`. Parent üzerinde unit uydurulmadı.
...
`src/designState.js > createCounterModuleState` `getItem(itemKey|width+shape)` ile canonical Item’dan default üretir; `itemKey` damgalar; 6 editable face parity korunur. Load’da `normalizeModuleItemState` `itemKey` hydrate eder; alias yok.
