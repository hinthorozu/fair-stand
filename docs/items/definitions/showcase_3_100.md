# showcase_3_100 — Item Contract Definition
...
Eski `wall_showcase_100_3` catalog key aktif ürün kimliği değildir. Restore compatibility resolver eski key'i `showcase_3_100` canonical kimliğine çevirir; yeni state/catalog yalnız canonical key'i yazar.
...
Diğer base child Item miktarları değişmez. Variant `src/moduleRecipes.js` içinde tek canonical recipe kaynağından çözülür. `resolveItemBom('showcase_3_100', 1, { panelVariant: 'inner-corner' })` terminal Item satırlarını recursive üretir.
...
`showcase-3` mevcut shared `WALL_BEHAVIOR` contract'ını kullanır: wall placement, 50 cm move snap, 90° rotation, segment collision, standard magnetic snap, side insert, wall capacity ve shared ghost/reflow. Migration bu engine'leri yeniden yazmaz.
...
`createShowcaseModule()` body role resolver üzerinden canonical side/horizontal Item ölçülerini kullanır. 30 cm depth ve 18 mm thickness renderer'da bağımsız ürün sabiti değildir. Cam raf `glass_shelf` canonical Item ölçü/material'ından gelir; adet `2` ile `eyeCount-1` fiziksel raf düzenine eşittir.
