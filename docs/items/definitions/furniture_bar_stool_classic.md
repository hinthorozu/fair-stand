...
`src/items.js > FURNITURE_ITEMS.furniture_bar_stool_classic` canonical itemKey/name/type/dimensions sahibidir. Tekil Item; `catalogKey = itemKey = furniture_bar_stool_classic`. Type `bar-stool`; footprint 60×55×121 cm. Katalogda modelFile yoktu; renderer `bar_chair.glb` hardcode eder. Parent unit / BOM uydurulmadı (`decision-required`).

## Factory, state ve persistence
`createBarStoolModuleState` Item’dan default üretir; `itemKey`/`catalogKey` damgalar; editable `surface` korunur (`free-model-color`). Load’da `normalizeModuleItemState` hydrate eder; alias yok.
