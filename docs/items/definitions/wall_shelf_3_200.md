...
## Bileşim / BOM
`composition.mode=recipe`, `moduleType=shelf`; recipe lookup `item.dimensions.widthCm=200`, `options.shelfCount=3`. Miktar tek kaynak: `src/moduleRecipes.js` `shelf:200:3`. Alt satırları Item kartında kopyalanmaz; mevcut recipe miktarları değişmez. Raw BOM UI dokunulmadı.
