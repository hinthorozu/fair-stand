...
Bu belge `counter_top_110_60` Item'ının canonical Item Contract tanımını kaydeder. Migration öncesi mevcut kod zinciri `docs/items/current-system/counter_top_110_60.md` içindedir.
...
Canonical production kaydı `src/productionParts.js` içindeki `PRODUCTION_PARTS.counter_top_110_60` kaydıdır. Legacy `partId` kaldırılır; paralel ikinci ürün kimliği oluşturulmaz.
...
Parent counter state generic module state akışında persist edilir; `counter_top_110_60` ayrı production instance olarak saklanmaz.
...
Düz 100 ve L100 mevcut Raw BOM parser akışında resolve edilebilir.

## 4. Parent recipe kullanımı ve quantity ownership

`counter_top_110_60` tam **2** aktif parent recipe'de kullanılır:
...
Quantity sahibi parent recipe'dir. Migration miktarı değiştirmez; ilgili satırlar canonical `{ itemKey: 'counter_top_110_60', quantity: 1 }` kimliğine taşınır.

## 5. BOM / composition / pricing

`counter_top_110_60` başka Item'lardan oluşmaz; **Tekil Item**dır. Recipe expansion mevcut `getRecipeItemKey()` → `getProductionItem()` yolunu kullanır.
...
1. canonical `itemKey = counter_top_110_60`; legacy `partId` yoktur,
2. `type = counter-top`, `unit = adet`, `110 × 60 × 1.8 cm` metadata korunur,
