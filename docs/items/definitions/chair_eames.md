...
`src/items.js > FURNITURE_ITEMS.chair_eames` canonical itemKey/name/type/dimensions sahibidir. Tekil Item; `catalogKey = itemKey = chair_eames`. Type `chair`; footprint 46×58×82 cm. Katalogda modelFile yok; renderer mevcut `eames_chair.glb` yükler. Unit / BOM uydurulmadı (`decision-required`).

## Factory, state ve persistence
`createEamesChairModuleState` Item’dan default üretir; `itemKey`/`catalogKey` damgalar; editable `surface` korunur (`free-model-color`). Load’da `normalizeModuleItemState` hydrate eder; alias yok.
