...
`src/designState.js > createTvModuleState` canonical Item'ı `resolveWallMediaMetrics` ile çözer; toplam ölçüler panel × grid'den derive edilir. Instance `id`, `itemKey`, `type`, türetilmiş toplam ölçüler, `sizeInch`, `videoWallRows/Cols=2` ve panel ölçülerini taşır; `heightCm` toplam ekran yüksekliğidir. `tv` state factory tek oluşturma yoludur; dış descriptor ürün defaultlarını tanımlayamaz. Proje load'unda `normalizeModuleItemState` `itemKey` hydration'ı yapar; `resolveItemKey` legacy state'i `widthCm/sizeInch` üzerinden `VIDEO_WALL_2X2`'ye çözer. Alias/dual-write yoktur.
