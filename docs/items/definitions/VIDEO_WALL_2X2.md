...
`src/moduleBehavior.js > tv: overlayBehavior()` placement/snap/collision type ailesidir: `wall-overlay`, move snap `10 cm`, collision/magnetic snap `none`. Sahne Z Item kolonlarıdır (`docs/refactor/ROTATION.md`). `src/scene3d.js > createTvModule()` tek örnek olarak `2×2` grid için 1 cm black seam mesh'leri ekler; bu görsel temsildir, business material/BOM değildir.
