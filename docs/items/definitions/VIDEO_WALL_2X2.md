...
`src/items.js > WALL_MEDIA_ITEMS.VIDEO_WALL_2X2` kanonik itemKey/name/type/sizeInch/videoWall/dimensions sahibidir. **Tekil parametrik Item'dır**; parametreler `videoWall.rows × cols` grid'i ve doğrulanmış panel ölçüsüdür (`panelScreenWidthCm=108.5`, `panelScreenHeightCm=61`). Toplam ekran/modül ölçüleri panel × grid'den türetilir (`217 × 122 cm`); `depthCm=5`, `sizeInch=55`. `itemKey = VIDEO_WALL_2X2`. Bu Item N adet ayrı `TV_55`'in bileşimi DEĞİLDİR; composite BOM üretilmez. Doğrulanmış unit YOK; unit/malzeme/renk tahmin edilmedi.

## Oluşturma, state ve kalıcılık
`src/designState.js > createTvModuleState` kanonik Item'ı `resolveWallMediaMetrics` ile çözer; toplam ölçüler panel × grid'den derive edilir. Örnek `id`, `itemKey`, `type`, türetilmiş toplam ölçüler, `sizeInch`, `videoWallRows/Cols=2` ve panel ölçülerini taşır; `heightCm` toplam ekran yüksekliğidir. `tv` state oluşturucu tek oluşturma yoludur; dış tanım ürün defaultlarını tanımlayamaz. Proje load'unda `normalizeModuleItemState` `itemKey` doldurur; `resolveItemKey` eski kaydı `widthCm/sizeInch` üzerinden `VIDEO_WALL_2X2`'ye çözer. Takma ad /çift yazım yoktur.
