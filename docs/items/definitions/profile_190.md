...
```js
{
  ...
  type: 'profile',
  unit: 'adet',
  dimensions: { widthCm: 190, depthCm: 8, heightCm: 8 },
  material: 'alüminyum',
...
## 5. BOM / composition
Tekil production Item; 7 doğrulanmış parent recipe (wall/shelf/counter/L-counter/base/base-wall). Recipe identity kanonik `itemKey` kullanır, quantity parent recipe sahibidir, expansion üstveriyi `getItem()` üzerinden çözer. Saha örneği `resolveItemBom` ile `profile_190 × 1 adet` üretir; parent reçetelere otomatik eklenmez.
...
Regression `itemKey`, `type`, `unit`, `190 × 8 cm`, `material='alüminyum'`, `defaultColor=0xd0d3d4`, 7 parent recipe eşyapı ve kanonik expansion'ı korur.
