...
`src/items.js > COMPOSITE_ITEMS.desk_banko_100_L` canonical itemKey/name/type/shape/dimensions sahibidir. Bileşik Item; `itemKey = desk_banko_100_L`. Type ailesi `counter`; `shape=L`; ölçü 100×100×100 cm. Parent üzerinde unit uydurulmadı.
...
`createCounterModuleState` Item’dan default üretir; 8 editable face. Load hydrate `itemKey`; alias yok.
