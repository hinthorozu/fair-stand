...
## Bileşim / BOM
`composition.mode=recipe`, `moduleType=counter`; recipe lookup `item.dimensions.widthCm=100`, `options.shape=L`. Miktar tek kaynak: `src/moduleRecipes.js` → `counter-l:100`. Alt satırları Item kartında kopyalanmaz; mevcut recipe miktarları bu migration’da değişmez.
...
Type `counter` placement/snap. Sahne Z Item kolonlarıdır (`docs/refactor/ROTATION.md`). `createLCounterModule` prosedürel; iş tek kaynağı değildir.
