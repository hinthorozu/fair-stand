...
## 5. BOM / composition
Tekil production Item; 12 doğrulanmış parent recipe (wall/door/shelf/showcase/separator/counter/L-counter/base/base-wall). Recipe identity canonical `itemKey` kullanır, quantity parent recipe sahibidir, expansion metadata'yı `getProductionItem()` üzerinden çözer.
...
Regression `itemKey`, `type`, `unit`, `91 × 8 cm`, `material='alüminyum'`, `defaultColor=0xd0d3d4`, 12 parent recipe parity ve canonical expansion'ı korur.
