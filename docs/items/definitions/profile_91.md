...
## 5. BOM / composition
Tekil production Item; 12 doğrulanmış parent recipe (wall/door/shelf/showcase/separator/counter/L-counter/base/base-wall). Recipe identity kanonik `itemKey` kullanır, quantity parent recipe sahibidir, expansion üstveriyi `getProductionItem()` üzerinden çözer. Saha örneği `resolveItemBom` ile `profile_91 × 1 adet` üretir; parent reçetelere otomatik eklenmez.
...
Regression `itemKey`, `type`, `unit`, `91 × 8 cm`, `material='alüminyum'`, `defaultColor=0xd0d3d4`, 12 parent recipe eşyapı ve kanonik expansion'ı korur.
