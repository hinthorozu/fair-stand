...
## Bileşim / BOM
`composition.mode=recipe`, `moduleType=base`; recipe lookup `item.dimensions.widthCm=150`. Miktar tek kaynak: `src/moduleRecipes.js` → `base:150`. `wall_base_150` ayrı type/recipe (`base-wall:150`); aynı `base_top_157_50` satırını paylaşır — reçete/quantity bu migration’da değişmez.
