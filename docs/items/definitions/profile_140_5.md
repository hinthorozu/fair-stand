...
## 5. BOM / composition
Tekil production Item; 8 doğrulanmış parent recipe (wall/shelf/counter/L-counter/base/base-wall). Recipe identity canonical `itemKey` kullanır, quantity parent recipe sahibidir, expansion metadata'yı `getProductionItem()` üzerinden çözer.
...
Regression `itemKey`, `type`, `unit`, `140.5 × 8 cm`, `material='alüminyum'`, `defaultColor=0xd0d3d4`, 8 parent recipe parity ve canonical expansion'ı korur.
