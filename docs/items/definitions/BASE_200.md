...
## Bileşim / BOM
`composition.mode=recipe`, `moduleType=base`; recipe lookup `item.dimensions.widthCm=200`. Miktar tek kaynak: `src/moduleRecipes.js` → `base:200`. `base_top_206_50` bu recipe’de ×1 tüketilir.
