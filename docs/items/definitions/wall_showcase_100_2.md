# wall_showcase_100_2 — Mevcut Sistem Profili

Bu belge `wall_showcase_100_2` için `Version2` runtime kodunda bulunan state, renderer, behavior, interaction, persistence ve BOM akışlarını toplar.

## Kimlik / factory

| Alan | Kod değeri |
|---|---|
| Catalog key | `wall_showcase_100_2` |
| Label | `2 Gözlü Vitrin 100` |
| Type | `showcase-2` |
| Width | `100 cm` |
| Eye count | `2` |

Factory `createShowcaseModuleState('showcase-2', 100)` ile **7 strip state** oluşturur. Her strip default beyaz, `imageAssetId=null`, default single image transform taşır.
...
```text
state strip count = 7
opening skipped strip indices = [2, 3]
rendered editable panel strip indices = [0, 1, 4, 5, 6]
rendered editable panel count = 5
...
Renderer'da yatay glass-shelf mesh döngüsü `index=1; index<eyeCount` çalışır; bu `2` gözlü varyant için **1 adet** glass-shelf mesh üretir.

Recipe tarafında `glass_shelf` adedi **2** olarak kayıtlıdır. Renderer mesh adedi ile recipe quantity kodda bu iki ayrı değerdir.
...
`duplicateModuleState()` state'i JSON clone eder, yeni module `id` üretir ve varsa strip/face/surface kimliklerini yeniler. `wall_showcase_100_2` için clone edilen normal nested state alanları korunur.
...
`buildProjectSnapshot()` bütün `currentModules` dizisini JSON clone ile proje snapshot'ındaki `modules` alanına yazar. `wall_showcase_100_2` state'i placement ve nested state alanlarıyla birlikte burada saklanır.
...
| `panel_98` | Panel 98 × 47 cm | 5 | adet |
| `connector_start` | Başlangıç Aparatı | 4 | adet |
| `connector_single` | Tekli Aparat | 9 | adet |
| `showcase_2_100` | 2 Gözlü Vitrin 100 cm | 1 | adet |
| `glass_shelf` | Cam Raf | 2 | adet |

Recipe metadata'sında ayrıca:

```text
variants.innerCornerPanelItemKey = panel_corner_92
```
...
`moduleDragSidebar.js` içinde `wall_showcase_100_2` katalog grubu **Raf & Vitrin** altında yer alır. Catalog card drag akışı `Shift+R` ile behavior rotation step'ini kullanır; ghost/placement preview `scene3d.previewCatalogModuleDrag()` yoluna gider.

`selectionFeedback.js` bu Item için kullanıcıya şu bilgi sınıfını üretir: `2 Gözlü Vitrin 100 cm · alttan <strip>. panel · renk + görsel uygulanabilir.`
