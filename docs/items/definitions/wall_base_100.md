...
## Bileşim / BOM
`composition.mode=recipe`, `moduleType=base-wall`; recipe lookup `item.dimensions.widthCm=100`. Miktar tek kaynak: `src/moduleRecipes.js` `base-wall:100` (`base-wall-100`). Alt satırları Item kartında kopyalanmaz; mevcut recipe miktarları bu migration’da değişmez. Raw BOM UI dokunulmadı.
