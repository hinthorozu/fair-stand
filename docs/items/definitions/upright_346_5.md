...
Katalog grubu `Panel Ek Modül`. Snap yalnız `short-up-1` / `short-up-2` uçları ve köşesi (`magneticSnap: short-up-joint`). Snap yoksa bırakılmaz. Düz `wall_*` kimliği ve reçetesi değişmez.
...
`test/upright3465ItemContract.test.js` 18 recipe ×2 miktarını korur. `test/uprightFieldPlacement.test.js` katalog, self BOM ve short-up-only snap'i korur.
