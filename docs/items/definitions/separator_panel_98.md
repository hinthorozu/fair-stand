...
Leaf Item bağımsız project instance değildir. Parent `separator` state'i oluşturulurken `src/designState.js#createSeparatorModuleState(100)` canonical Item `defaultColor` değerini okur ve UI/render state formatı olan `#c79b63` değerine dönüştürür.
...
- `separator:50 ×3` ve `separator:100 ×7` quantity parity,
