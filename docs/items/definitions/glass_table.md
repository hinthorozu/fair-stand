...
`src/items.js > FURNITURE_ITEMS.glass_table` canonical itemKey/name/type/dimensions sahibidir. Tekil Item; `itemKey = glass_table`. Type `table-glass`; çap `tableDiameterCm=75`; footprint 75×75×74 cm. ModelFile yok; renderer procedural cam masa. Unit / BOM uydurulmadı (`decision-required`).

## Factory, state ve persistence
`createGlassTableModuleState` Item’dan default üretir; `itemKey` damgalar; surface yok (`free-model-fixed`). Load’da `normalizeModuleItemState` hydrate eder; alias yok.
