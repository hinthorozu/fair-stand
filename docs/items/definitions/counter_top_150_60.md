...
Bu belge `counter_top_150_60` Item'ının canonical Item Contract tanımını kaydeder. Migration öncesi mevcut kod zinciri `docs/items/current-system/counter_top_150_60.md` içindedir.
...
Canonical production kaydı `src/productionParts.js` içindeki `PRODUCTION_PARTS.counter_top_150_60` kaydıdır. Legacy `partId` kaldırılır; paralel ikinci ürün kimliği oluşturulmaz.
...
Parent counter state generic module state akışında persist edilir; `counter_top_150_60` ayrı production instance olarak saklanmaz.
...
L200 selection metni Raw BOM adapter'ında `counter-l-200` recipe'sine resolve edilir. Bu UI bağlantısı parent/debug katmanına aittir; leaf Item quantity ownership değişmez.

## 4. Parent recipe kullanımı ve quantity ownership

`counter_top_150_60` tam **1** aktif parent recipe'de kullanılır:

| Recipe | Quantity |
|---|---:|
| `counter-l-200` | 1 |

Quantity sahibi parent recipe'dir. Migration miktarı değiştirmez; ilgili satırlar canonical `{ itemKey: 'counter_top_150_60', quantity: 1 }` kimliğine taşınır.

## 5. BOM / composition / pricing

`counter_top_150_60` başka Item'lardan oluşmaz; **Tekil Item**dır. Recipe expansion mevcut `getRecipeItemKey()` → `getProductionItem()` yolunu kullanır.
...
1. canonical `itemKey = counter_top_150_60`; legacy `partId` yoktur,
2. `type = counter-top`, `unit = adet`, `150 × 60 × 1.8 cm` metadata korunur,
