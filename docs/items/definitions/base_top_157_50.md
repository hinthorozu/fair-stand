...
Bu belge `base_top_157_50` Item'ının canonical Item Contract tanımını kaydeder. Migration öncesi mevcut kod zinciri `docs/items/current-system/base_top_157_50.md` içindedir.

## 1. Kimlik ve sınıflandırma

| Alan | Canonical değer | Durum |
|---|---|---|
| `itemKey` | `base_top_157_50` | VAR |
| `type` | `base-top` | VAR |
| Yapı | Tekil Item | VAR |
| Parametrik | hayır | UYGULANMIYOR |
| `unit` | `adet` | VAR |
| `dimensions.widthCm` | `157` | VAR |
| `dimensions.depthCm` | `50` | VAR |
| `dimensions.thicknessCm` | `1.8` | VAR — ürün kararıyla doğrulandı |
| `nominalModuleWidthCm` | `150` | VAR |

Canonical production kaydı `src/productionParts.js` içindeki `PRODUCTION_PARTS.base_top_157_50` kaydıdır. Legacy `partId` kaldırılır; paralel ikinci ürün kimliği oluşturulmaz.
...
- Doğrudan catalog kaydı: **UYGULANMIYOR**; parent catalog Item'ları `BASE_150` ve `wall_base_150`'dır.
- Ayrı mutable Item state veya project `id`: **UYGULANMIYOR**.
- Ayrı persistence entity/schema/save-load migration: **UYGULANMIYOR**.

Parent base/base-wall state mevcut generic module state akışında persist edilir; `base_top_157_50` ayrı production instance olarak saklanmaz.
...
`base_top_157_50` tam **2** aktif parent recipe'de kullanılır:
...
Quantity sahibi parent recipe'dir. Migration miktarı değiştirmez; iki satır da canonical `{ itemKey: 'base_top_157_50', quantity: 1 }` kimliğine taşınır.

## 5. BOM / composition / pricing

`base_top_157_50` başka Item'lardan oluşmaz; **Tekil Item**dır. Recipe expansion mevcut `getRecipeItemKey()` → `getProductionItem()` yolunu kullanır.
...
1. canonical `itemKey = base_top_157_50`; legacy `partId` yoktur,
2. `type = base-top`, `unit = adet`, `157 × 50 × 1.8 cm` metadata korunur,
