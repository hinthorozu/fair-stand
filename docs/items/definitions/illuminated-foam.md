...
`src/items.js > NON_CATALOG_ITEMS['illuminated-foam']` kanonik itemKey/name/type/dimensions sahibidir. Tekil Item. `itemKey = type = illuminated-foam`. Katalog kartı yoktur (`MODULE_CATALOG` dışı; `NON_CATALOG_MODULE_CONTRACTS`). Oluşturucu default ölçü: width `200`, height `50`, depth `3.5`, `wallGapCm=1.5`. Dialog limitleri mevcut UI’dadır (`10..5000` / `5..350`). Unit / BOM uydurulmadı (`decision-required`).
