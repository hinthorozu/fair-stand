...
Oluşturma yolu SVG asset sağ tık `Işıklı Strafora Dönüştür`. `createIlluminatedFoamModuleState` Item defaultlarını ve `itemKey` damgasını yazar; `catalogKey` yazılmaz. `imageAssetId` zorunludur. `haloColor` default `#ffffff`. Load hydrate `itemKey` doldurur, varsa `catalogKey` silinir. `Tüm Özellikleri Kaldır` bu type’ı sahneden çıkarır.
