...
`src/items.js > COMMERCIAL_ITEMS.PLASTIC_TRASH_BIN` kanonik itemKey/name/type/dimensions/modelFile sahibidir. Tekil ticari Item; parametrik veya bileşik değildir. Mevcut sistemde doğrulanmış unit YOK; BOM aşamasına geçilmediği için unit tahmin edilmedi. Width/depth/height mevcut katalog/state değerlerinden taşındı; length/thickness/material/defaultColor için doğrulanmış business üstveri YOK, tahmin eklenmedi. `modelRotationYDeg`, `visualRotationYDeg=-90` ve `preserveModelScale` defaultları aynı Item'dadır.
...
`test/commercialItemsContract.test.js`: özellikler, değişmeyen `decision-required` BOM politikası, state/save-load/duplicate, seçim metni, otomatik depo, dış tanım ürün alanlarının reddi ve diğer Item'ların migration izolasyonu.
