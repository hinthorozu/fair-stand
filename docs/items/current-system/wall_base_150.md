...
# wall_base_150 — Tarihî envanter profili

Bu belge `wall_base_150` için `Version2` runtime kodunda çalışan state, behavior, renderer, interaction, persistence ve BOM akışlarını toplar.
...
`duplicateModuleState()` state'i JSON clone eder, yeni module `id` üretir ve varsa strip/face/surface kimliklerini yeniler. `wall_base_150` için clone edilen normal nested state alanları korunur.
...
|---|---|---:|---|
| `profile_140_5` | Profil 140,5 cm | 4 | adet |
...
| `base_top_157_50` | Baza Üstü 157 × 50 cm | 1 | adet |
...
`moduleDragSidebar.js` içinde `wall_base_150` katalog grubu **Panel & Duvar** altında yer alır. Catalog card drag akışı `Shift+R` ile behavior rotation step'ini kullanır; ghost/placement preview `scene3d.previewCatalogModuleDrag()` yoluna gider.
