> Migration öncesi envanterdir; aktif canonical tanım `../definitions/wall_separator_100_sarmasik.md` içindedir.
...
| Alan | Kod değeri |
|---|---|
| Catalog key | `wall_separator_100_sarmasik` |
| Label | `Separatör 100 Sarmaşık` |
| Type | `separator` |
| Width | `100 cm` |
| Model file | `wall_separator_100_sarmasik.glb` |
...
catalogKey = wall_separator_100_sarmasik
```
...
Sarmaşık varyantında `wall_separator_100_sarmasik.glb` ayrıca GLB olarak yüklenir. Renderer modelde yalnız adı `Vines_FIXED_` ile başlayan mesh'leri görünür bırakır; bu dekoratif mesh'lerde raycast devre dışıdır. Model yüklenemezse veya uygun mesh bulunmazsa console warning üretilir. Procedural separator frame/slat yapısı yine oluşturulur.
...
`duplicateModuleState()` state'i JSON clone eder, yeni module `id` üretir ve varsa strip/face/surface kimliklerini yeniler. `wall_separator_100_sarmasik` için clone edilen normal nested state alanları korunur.
...
`buildProjectSnapshot()` bütün `currentModules` dizisini JSON clone ile proje snapshot'ındaki `modules` alanına yazar. `wall_separator_100_sarmasik` state'i placement ve nested state alanlarıyla birlikte burada saklanır.
...
| `separator_panel_98` | Separatör Paneli 98 × 47 cm | 7 | adet |
...
`moduleDragSidebar.js` içinde `wall_separator_100_sarmasik` katalog grubu **Panel & Duvar** altında yer alır. Catalog card drag akışı `Shift+R` ile behavior rotation step'ini kullanır; ghost/placement preview `scene3d.previewCatalogModuleDrag()` yoluna gider.

`selectionFeedback.js` bu Item için kullanıcıya şu bilgi sınıfını üretir: `Separatör 100 cm · yalnızca renk uygulanabilir.`
