...
Renderer `public/models/coat_rack.glb` yükler, modeli hedef 180 cm yüksekliğe uniform scale eder ve selection proxy oluşturur. State 43×43×180 cm'dir; image/color editable surface yoktur.
...
Automatic depot `includeContents` aktifken coat-rack spec'i depot içeriğine eklenir.
...
`duplicateModuleState()` state'i JSON clone eder, yeni module `id` üretir ve varsa strip/face/surface kimliklerini yeniler. `DEPOT_COAT_RACK` için clone edilen normal nested state alanları korunur.
...
`moduleDragSidebar.js` içinde `DEPOT_COAT_RACK` katalog grubu **Extra** altında yer alır. Catalog card drag akışı `Shift+R` ile behavior rotation step'ini kullanır; ghost/placement preview `scene3d.previewCatalogModuleDrag()` yoluna gider.
