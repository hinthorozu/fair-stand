...
## Kimlik / state (takım içinden)

| Alan | Kod değeri |
|---|---|
| Parent catalog key | `furniture_sofa_set_classic` |
| Genişlik | `60 cm` (`tableWidthCm`) |
| Derinlik | `42 cm` (`tableDepthCm`) |
| Yükseklik | `38 cm` (`tableHeightCm`) |
| Renk | yok; sehpa sabittir |

Tabla / sap / taban renderer mesh’tir; ayrı Item değildir. `glass_table` değildir.
