...
# wall_shelf_3_100 — Tarihî envanter profili

Bu belge `wall_shelf_3_100` için `Version2` runtime kodunda çalışan bütün ana akışları toplar.
...
`duplicateModuleState()` state'i JSON clone eder, yeni module `id` üretir ve varsa strip/face/surface kimliklerini yeniler. `wall_shelf_3_100` için clone edilen normal nested state alanları korunur.
...
`panel_corner_92` production registry kaydı: `İç Köşe Paneli 92 × 47 cm` (widthCm=92, heightCm=47, depthCm=0.8). Bu part normal `recipe.items` satırlarında yer almaz.
...
`moduleDragSidebar.js` içinde `wall_shelf_3_100` katalog grubu **Raf & Vitrin** altında yer alır. Catalog card drag akışı `Shift+R` ile behavior rotation step'ini kullanır; ghost/placement preview `scene3d.previewCatalogModuleDrag()` yoluna gider.
