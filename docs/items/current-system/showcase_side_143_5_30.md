...
| 14 | BOM / composition | VAR | Tekil Item; `resolveItemBom()` ile doğrudan canonical BOM satırı üretir. Parent `showcase_3_100` içindeki quantity/cutover canonical parent recipe içinde tanımlıdır. |
...
| 18 | Open decisions / completion | KAPALI | Ürün property kararı kapalıdır. Bireysel renk edit'i bilinçli olarak yoktur. Parent recipe, renderer consumption ve tek gövde renk override'ı `showcase_3_100` migrationında tamamlanacaktır. |
