...
|---:|---|---|---|
| 1 | Identity / type | VAR | `itemKey=showcase_side_143_5_30`, `type=showcase-board`, `unit=adet`; canonical owner `src/productionParts.js`. |
...
| 4 | Factory / creation | PARENT-OWNED | Ayrı leaf project factory yok. Parent `wall_showcase_100_3` / showcase module factory instance'ı üretir. |
...
| 14 | BOM / composition | VAR | Tekil Item; `resolveItemBom()` ile doğrudan canonical BOM satırı üretir. Parent `wall_showcase_100_3` içindeki quantity/cutover bir sonraki parent migrationında tanımlanacaktır. |
...
`showcase_side_143_5_30` yeni canonical physical Item olarak tanımlandı. Ölçü, sunta material ve beyaz default artık `PRODUCTION_PARTS` kaydında tek ürün gerçeğidir.
