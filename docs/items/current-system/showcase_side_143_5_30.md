...
| 4 | Factory / creation | PARENT-OWNED | Ayrı leaf project factory yok. Parent `showcase_3_100` / showcase module factory instance'ı üretir. |
...
| 14 | BOM / composition | VAR | Tekil Item; `resolveItemBom()` ile doğrudan canonical BOM satırı üretir. Parent `showcase_3_100` içindeki quantity/cutover bir sonraki parent migrationında tanımlanacaktır. |
...
| 18 | Open decisions / completion | LEAF KAPALI / PARENT ENTEGRASYON BEKLİYOR | Ürün property kararı kapalıdır. Bireysel renk edit'i bilinçli olarak yoktur. Parent recipe, renderer consumption ve tek gövde renk override'ı `showcase_3_100` migrationında tamamlanacaktır. |
...
Mevcut `showcase_3_100` legacy recipe ve procedural renderer bu child-only batch'te değiştirilmez.