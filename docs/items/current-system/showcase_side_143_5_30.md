...
|---:|---|---|---|
| 1 | Identity / type | VAR | `itemKey=showcase_side_143_5_30`, `type=showcase-board`, `unit=adet`; kanonik owner `src/productionParts.js`. |
| 2 | Intrinsic properties | VAR | `143.5 × 30 × 1.8 cm` (`length × depth × thickness`), `material=sunta`, `defaultColor=0xffffff`; kullanıcı ürün kararı. |
...
| 4 | Oluşturma | PARENT-OWNED | Ayrı leaf project factory yok. Parent `wall_showcase_100_3` / showcase module factory instance'ı üretir. |
...
| 14 | BOM / composition | VAR | Tekil Item; `resolveItemBom()` ile doğrudan kanonik BOM satırı üretir. Parent `wall_showcase_100_3` içindeki quantity/cutover bir sonraki parent migrationında tanımlanacaktır. |
| 15 | Renderer / asset / override boundary | PARENT-OWNED / ENTEGRASYON BEKLİYOR | Ayrı leaf renderer yok. Mevcut procedural showcase renderer yan gövdeyi parent seviyesinde çizer. Kanonik board ölçü/defaultColor tüketimi parent showcase migrationında bağlanacaktır. |
...
| 18 | Open decisions / completion | LEAF KAPALI / PARENT ENTEGRASYON BEKLİYOR | Ürün property kararı kapalıdır. Bireysel renk edit'i bilinçli olarak yoktur. Parent recipe, renderer consumption ve tek gövde renk override'ı `wall_showcase_100_3` migrationında tamamlanacaktır. |

## Kanonik cutover

`showcase_side_143_5_30` yeni kanonik physical Item olarak tanımlandı. Ölçü, sunta material ve beyaz default artık `PRODUCTION_PARTS` kaydında tek ürün gerçeğidir.
...
Mevcut `wall_showcase_100_3` legacy recipe ve procedural renderer bu child-only batch'te değiştirilmez.