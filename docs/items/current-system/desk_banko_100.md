...
# desk_banko_100 — Tarihî envanter profili

Bu belge `desk_banko_100` için `Version2` runtime kodunda çalışan state, placement, renderer, interaction, persistence ve BOM akışlarını toplar.
...
- width `100 cm`, depth `50 cm`, height `100 cm`,
- 4 corner post,
...
`duplicateModuleState()` state'i JSON clone eder, yeni module `id` üretir ve varsa strip/face/surface kimliklerini yeniler. `desk_banko_100` için clone edilen normal nested state alanları korunur.
...
|---|---|---:|---|
| `profile_91` | Profil 91 cm | 3 | adet |
...
`moduleDragSidebar.js` içinde `desk_banko_100` katalog grubu **Banko & Baza** altında yer alır. Catalog card drag akışı `Shift+R` ile behavior rotation step'ini kullanır; ghost/placement preview `scene3d.previewCatalogModuleDrag()` yoluna gider.
