> Migration öncesi envanterdir; aktif kanonik tanım `../definitions/desk_banko_150_L.md` içindedir.

# desk_banko_150_L — Tarihî envanter profili

Bu belge `desk_banko_150_L` için `Version2` runtime kodunda çalışan state, placement, renderer, interaction, persistence ve BOM akışlarını toplar.

## Kimlik / state

| Alan | Kod değeri |
|---|---|
| Katalog anahtarı | `desk_banko_150_L` |
| Etiket | `Köşe Banko 150×150` |
| Type | `counter` |
| Shape | `L` |
| Genişlik | `150 cm` |
| Derinlik | `150 cm` |
...
Top geometri width/depth ve 2 cm overhang üzerinden procedural oluşturulur.
...
`duplicateModuleState()` state'i JSON clone eder, yeni module `id` üretir ve varsa strip/face/surface kimliklerini yeniler. `desk_banko_150_L` için clone edilen normal nested state alanları korunur.
...
`buildProjectSnapshot()` bütün `currentModules` dizisini JSON clone ile proje snapshot'ındaki `modules` alanına yazar. `desk_banko_150_L` state'i placement ve nested state alanlarıyla birlikte burada saklanır.
...
| `counter_top_102_60` | Banko Üstü 102 × 60 cm | 1 | adet |
...
`moduleDragSidebar.js` içinde `desk_banko_150_L` katalog grubu **Banko & Baza** altında yer alır. Catalog card drag akışı `Shift+R` ile behavior rotation step'ini kullanır; ghost/placement preview `scene3d.previewCatalogModuleDrag()` yoluna gider.
