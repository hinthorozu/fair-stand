...
|---:|---|---|---|
| 1 | Identity / type | VAR | `itemKey=shelf_200`, `type=shelf`, `unit=adet`; kanonik owner `src/productionParts.js`. |
...
| 12 | Kalıcılık | PARENT-OWNED | `main.js` + `projectStore.js` parent module state'ini persist/restore eder. Leaf `shelf_200` ayrı persisted entity değildir; kanonik product default'ları persistence'a kopyalanmaz. |
