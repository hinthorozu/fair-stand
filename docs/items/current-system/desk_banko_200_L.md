...
# desk_banko_200_L — Tarihî envanter profili

Bu belge `desk_banko_200_L` için `Version2` runtime kodunda çalışan state, placement, renderer, interaction, persistence ve BOM akışlarını toplar.
...
`duplicateModuleState()` state'i JSON clone eder, yeni module `id` üretir ve varsa strip/face/surface kimliklerini yeniler. `desk_banko_200_L` için clone edilen normal nested state alanları korunur.
...
|---|---|---:|---|
| `profile_190` | Profil 190 cm | 5 | adet |
| `profile_140_5` | Profil 140,5 cm | 1 | adet |
| `profile_41_5` | Profil 41,5 cm | 4 | adet |
| `upright_99` | Dikme 99 cm | 5 | adet |
| `panel_197` | Panel 197 × 47 cm | 4 | adet |
...
| `counter_top_150_60` | Banko Üstü 150 × 60 cm | 1 | adet |
...
`moduleDragSidebar.js` içinde `desk_banko_200_L` katalog grubu **Banko & Baza** altında yer alır. Catalog card drag akışı `Shift+R` ile behavior rotation step'ini kullanır; ghost/placement preview `scene3d.previewCatalogModuleDrag()` yoluna gider.
