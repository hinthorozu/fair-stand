> Migration öncesi envanterdir; aktif kanonik tanım `../definitions/wall_base_200.md` içindedir.

# wall_base_200 — Tarihî envanter profili

Bu belge `wall_base_200` için `Version2` runtime kodunda çalışan state, behavior, renderer, interaction, persistence ve BOM akışlarını toplar.

## Kimlik / state

| Alan | Kod değeri |
|---|---|
| Katalog anahtarı | `wall_base_200` |
| Etiket | `Panel Bazalı 200` |
| Type | `base-wall` |
| Genişlik | `200 cm` |
| Derinlik | `50 cm` |
| Yükseklik | `350 cm` |
...
`duplicateModuleState()` state'i JSON clone eder, yeni module `id` üretir ve varsa strip/face/surface kimliklerini yeniler. `wall_base_200` için clone edilen normal nested state alanları korunur.
...
`buildProjectSnapshot()` bütün `currentModules` dizisini JSON clone ile proje snapshot'ındaki `modules` alanına yazar. `wall_base_200` state'i placement ve nested state alanlarıyla birlikte burada saklanır.
...
|---|---|---:|---|
| `profile_190` | Profil 190 cm | 4 | adet |
...
| `base_top_206_50` | Baza Üstü 206 × 50 cm | 1 | adet |
...
`moduleDragSidebar.js` içinde `wall_base_200` katalog grubu **Panel & Duvar** altında yer alır. Catalog card drag akışı `Shift+R` ile behavior rotation step'ini kullanır; ghost/placement preview `scene3d.previewCatalogModuleDrag()` yoluna gider.
