> Migration öncesi envanterdir; aktif kanonik tanım `../definitions/EXTRA_LONG_PLANTER_200.md` içindedir.

# EXTRA_LONG_PLANTER_200 — Tarihî envanter profili

Bu belge `EXTRA_LONG_PLANTER_200` için `Version2` runtime kodunda bulunan state, behavior, renderer, interaction ve persistence akışlarını toplar.

## Kimlik / state

| Alan | Kod değeri |
|---|---|
| Katalog anahtarı | `EXTRA_LONG_PLANTER_200` |
| Etiket | `Uzun Saksı 200` |
| Type | `indoor-plant-1` |
| Genişlik | `200 cm` |
| Derinlik | `30 cm` |
| Yükseklik | `30 cm` |
| Model dosyası | `saksi_bitkili_200x30x30.glb` |
...
```text
{
  ...
  "catalogKey": "EXTRA_LONG_PLANTER_200"
}
```
...
Renderer `createIndoorPlantModule()` üzerinden `saksi_bitkili_200x30x30.glb` yükler. `preserveModelScale=true`, `modelRotationYDeg=90` state'indedir. Renderer planter/pot mesh adlarını tespit edip bunları `colorTargets` olarak surface state'e bağlar; image editable değildir. Selection module seviyesindedir.
...
`duplicateModuleState()` state'i JSON clone eder, yeni module `id` üretir ve varsa strip/face/surface kimliklerini yeniler. `EXTRA_LONG_PLANTER_200` için clone edilen normal nested state alanları korunur.
...
`buildProjectSnapshot()` bütün `currentModules` dizisini JSON clone ile proje snapshot'ındaki `modules` alanına yazar. `EXTRA_LONG_PLANTER_200` state'i placement ve nested state alanlarıyla birlikte burada saklanır.
...
`moduleDragSidebar.js` içinde `EXTRA_LONG_PLANTER_200` katalog grubu **Extra** altında yer alır. Catalog card drag akışı `Shift+R` ile behavior rotation step'ini kullanır; ghost/placement preview `scene3d.previewCatalogModuleDrag()` yoluna gider.
