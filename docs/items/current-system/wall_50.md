> Migration öncesi envanterdir; aktif kanonik tanım `../definitions/wall_50.md` içindedir.

# wall_50 — Tarihî envanter profili

Bu belge `wall_50` için `Version2` runtime kodunda bugün gerçekten bulunan state, behavior, placement, interaction, renderer, persistence ve BOM akışlarını toplar. Yeni davranış tanımlamaz; yorum/audit kaynağı kullanmaz.
...
Runtime state'te `itemKey` / `class` alanı yoktur. Kimlik için `catalogKey = wall_50` ve `type = flat-panel` kullanılır.

## Oluşturma / varsayılan state

`createFlatPanelModuleState(50)` temel olarak:
...
Wall snap mesafesi `50 cm`, module-neighbor snap mesafesi `30 cm`dir. Wall orientation değerleri `back=0°`, `left=90°`, `right=270°`dir. `wall_50` rotation step'i `90°`, move snap'i `50 cm`dir.
...
`composeStraightWall()` mevcut `MODULE_WIDTHS_CM = [50,100,150,200]` değerlerini büyükten küçüğe kullanır. Uygun stand uzunluklarında `50 cm` flat-panel otomatik stand duvarının parçası olarak üretilebilir.
...
`wall_50` GLB kullanmaz; `createFlatPanelModule()` ile procedural oluşturulur.
...
| Ara yatay rail | `yok` |
| Render panel iç genişliği | `40.8 cm` |
...
`supportsWallOverlayMount = true`. TV ve illuminated-foam gibi wall-overlay modüller duvara veya free placement'taki destek panelinin ön yüzüne bağlanabilir. Overlay modül `wall_50` recipe'sine otomatik eklenmez.
...
`duplicateModuleState()` state'i JSON clone eder, yeni module `id` üretir ve varsa strip/face/surface kimliklerini yeniler. `wall_50` için clone edilen normal nested state alanları korunur.
...
`buildProjectSnapshot()` bütün `currentModules` dizisini JSON clone ile proje snapshot'ındaki `modules` alanına yazar. `wall_50` state'i placement ve nested state alanlarıyla birlikte burada saklanır.
...
`panel_corner_42_5` production registry kaydı: `İç Köşe Paneli 42,5 × 47 cm` (widthCm=42.5, heightCm=47, depthCm=0.8). Bu part normal `recipe.items` satırlarında yer almaz.
...
`moduleDragSidebar.js` içinde `wall_50` katalog grubu **Panel & Duvar** altında yer alır. Catalog card drag akışı `Shift+R` ile behavior rotation step'ini kullanır; ghost/placement preview `scene3d.previewCatalogModuleDrag()` yoluna gider.
