...
Tekil Item. 12 doğrulanmış parent recipe (wall/door/shelf/showcase/separator/counter/L-counter/base/base-wall); recipe satırları kanonik `itemKey` kullanır, quantity parent recipe'den gelir ve expansion `getProductionItem()` ile metadata çözer.
