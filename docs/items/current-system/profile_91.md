...
## 1. Identity / type — VAR
`itemKey=profile_91`, `name=Profil 91 cm`, `type=profile`, `unit=adet`; owner `src/items.js`. Legacy `partId`: YOK.

## 2. Intrinsic properties — VAR
`widthCm=91`, `depthCm=8`, `heightCm=8`, `material='alüminyum'`, `defaultColor=0xd0d3d4`. Material kullanıcı ürün kararıdır. Mevcut render default'u `src/theme.js` içindeki `ALUMINUM_PROFILE_COLOR='#D0D3D4'` ile eşdeğerdir.
...
Tekil Item. 12 doğrulanmış parent recipe (wall/door/shelf/showcase/separator/counter/L-counter/base/base-wall); recipe satırları kanonik `itemKey` kullanır, quantity parent recipe'den gelir ve expansion `getItem()` ile metadata çözer.
