...
# VIDEO_WALL_2X2 — Tarihî envanter profili

Bu belge `VIDEO_WALL_2X2` için migration öncesi `Version2` runtime kodunda çalışan TV / wall-overlay akışını toplar. O dönemde katalog kaydı `TV_55` tabanından türetilip toplam ölçü/panel/grid alanları elle override ediliyordu (`src/catalog.js`).
...
`createTvModule()` procedural black TV box oluşturur ve front face'e `tv-screen.jpg` texture uygular. Bu video wall varyantında `2×2` panel grid'i için 1 cm black seam mesh'leri panel sınırlarına eklenir. Runtime tek instance'tır; N adet ayrı TV_55 değildir.
