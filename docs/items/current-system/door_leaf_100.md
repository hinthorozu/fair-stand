...
| 3 | Default state | VAR | Parent door state içindeki child surface `itemKey=door_leaf_100`, canonical beyazdan başlar. |
| 4 | Factory / creation | VAR | `createDoorModuleState(100)` → `getDoorLeafProductionItem(100)` → child editable surface. |
...
| 13 | Persistence | VAR | `surface.itemKey/color/imageAssetId/imageTransform` parent project state içinde saklanır; eski door surface state restore sırasında `door_leaf_100` ile normalize edilir. |
| 14 | Relationships / reflow | VAR | `door_leaf_100`, 100 cm door module'ünün fiziksel child/BOM Item'ıdır; bağımsız reflow yoktur. |
| 15 | BOM / composition | VAR | Parent `door:100` recipe `door_leaf_100 ×1`; unit canonical Item'dan `adet`. |
...
Renk ve görsel kullanıcı override'ıdır; canonical Item default'unu değiştirmez. `glass`, `lightbox` ve `mesh` bu Item için capability değildir.
