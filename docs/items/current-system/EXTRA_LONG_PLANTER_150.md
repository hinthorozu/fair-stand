...
# EXTRA_LONG_PLANTER_150 — Tarihî envanter profili

Bu belge `EXTRA_LONG_PLANTER_150` için `Version2` runtime kodunda bulunan state, behavior, renderer, interaction ve persistence akışlarını toplar.
...
```text
{
  ...
  "catalogKey": "EXTRA_LONG_PLANTER_150"
}
```
...
Renderer `createIndoorPlantModule()` üzerinden `saksi_bitkili_150x30x30.glb` yükler. `preserveModelScale=true`, `modelRotationYDeg=90` state'indedir. Renderer planter/pot mesh adlarını tespit edip bunları `colorTargets` olarak surface state'e bağlar; image editable değildir. Selection module seviyesindedir.
...
`duplicateModuleState()` state'i JSON clone eder, yeni module `id` üretir ve varsa strip/face/surface kimliklerini yeniler. `EXTRA_LONG_PLANTER_150` için clone edilen normal nested state alanları korunur.
...
`moduleDragSidebar.js` içinde `EXTRA_LONG_PLANTER_150` katalog grubu **Extra** altında yer alır. Catalog card drag akışı `Shift+R` ile behavior rotation step'ini kullanır; ghost/placement preview `scene3d.previewCatalogModuleDrag()` yoluna gider.
