...
# wall_150 — Tarihî envanter profili

Bu belge `wall_150` için `Version2` runtime kodunda bugün gerçekten bulunan state, behavior, placement, interaction, renderer, persistence ve BOM akışlarını toplar. Yeni davranış tanımlamaz; yorum/audit kaynağı kullanmaz.
...
Wall snap mesafesi `50 cm`, module-neighbor snap mesafesi `30 cm`dir. Wall orientation değerleri `back=0°`, `left=90°`, `right=270°`dir. `wall_150` rotation step'i `90°`, move snap'i `50 cm`dir.
...
`composeStraightWall()` mevcut `MODULE_WIDTHS_CM = [50,100,150,200]` değerlerini büyükten küçüğe kullanır. Uygun stand uzunluklarında `150 cm` flat-panel otomatik stand duvarının parçası olarak üretilebilir.
...
`wall_150` GLB kullanmaz; `createFlatPanelModule()` ile procedural oluşturulur.
...
| Ara yatay rail | `yok` |
| Render panel iç genişliği | `140.8 cm` |
...
`supportsWallOverlayMount = true`. TV ve illuminated-foam gibi wall-overlay modüller duvara veya free placement'taki destek panelinin ön yüzüne bağlanabilir. Overlay modül `wall_150` recipe'sine otomatik eklenmez.
...
`duplicateModuleState()` state'i JSON clone eder, yeni module `id` üretir ve varsa strip/face/surface kimliklerini yeniler. `wall_150` için clone edilen normal nested state alanları korunur.
...
```text
variants.innerCornerPanelItemKey = panel_corner_142_5
```

`panel_corner_142_5` production registry kaydı: `İç Köşe Paneli 142,5 × 47 cm` (widthCm=142.5, heightCm=47, thicknessCm=0.8). Bu part normal `recipe.items` satırlarında yer almaz.
...
`moduleDragSidebar.js` içinde `wall_150` katalog grubu **Panel & Duvar** altında yer alır. Catalog card drag akışı `Shift+R` ile behavior rotation step'ini kullanır; ghost/placement preview `scene3d.previewCatalogModuleDrag()` yoluna gider.
